...
BlacksmithLevel:
    ...
    script:
      ...
      - foreach <server.online_players> as:p:
        - define exp <[p].flag[Profil.Professions.Blacksmith.Exp]>
        - define exptarget <[p].flag[Profil.Professions.Blacksmith.Target]>
        - if <[Exp]> >= <[exptarget]>:
          - flag <[p]> Profil.Professions.Blacksmith.Level:++
          - flag <[p]> Profil.Professions.Blacksmith.Exp:-:<[p].flag[Profil.Professions.Blacksmith.Target]>
          - flag <[p]> Profil.Professions.Blacksmith.Target:*:2
          - narrate targets:<[p]> "<green><bold><script[Blacksmith].data_key[Blacksmith_<[p].flag[Profil.Professions.Blacksmith.Level].sub[1]>]> erhöhte sich auf <green><bold><script[Blacksmith].data_key[Blacksmith_<[p].flag[Profil.Professions.Blacksmith.Level]>]>" per_player
...
BlacksmithXPBar:
    ...
    - foreach <server.online_players> as:p:
        ...
        - flag <[p]> BlacksmithProzent:<[Prozent]>
        - repeat 20:
          - define zahl <[zahl].add[5]>
          - if <[zahl]> == 100:
            - if <[p].flag[BlacksmithProzent]> >= 99:
              - define finish <green>-
            ...
            - else:
              ...
              - define list <[list].include[<[finish]>]>
          - else:
            - if <[p].flag[BlacksmithProzent]> >= <[zahl]>:
              - define finish <green>-
              - define list <[list].include[<[finish]>]>
            - else:
              - define finish <white>-
              - define list <[list].include[<[finish]>]>
          - flag <[p]> Profil.ExpBar.Blacksmith:<[list].unseparated>
