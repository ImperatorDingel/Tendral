Friend:
    ...
    - choose <context.args.get[1]>:
        - case == list:
            - foreach <player.flag[Profil.Freundesliste.Freunde]> as:p:
                - define zahl 0
                - if <[p].is_online>:
                    - define color <green>
                    - define zahl:++
        ...
        - case == add:
            ...
            - if <context.args.get[2].if_null[NULL]> == NULL:
                ...
                - determine cancelled
            - if <[empfänger].flag[Profil.Freundesliste.Anfragen].contains[<[absender]>]>:
                - narrate "<script[Freunde_Data].parsed_key[Add3].separated_by[<&nl>]>"
                - determine cancelled
            - if <[empfänger].flag[Profil.Freundesliste.Freunde].contains[<[absender]>]>:
                - narrate "<script[Freunde_Data].parsed_key[Add4].separated_by[<&nl>]>"
                - determine cancelled
            - else:
                - flag <[absender]> Profil.Freundesliste.Ausgang:->:<[empfänger]>
                - flag <[empfänger]> Profil.Freundesliste.Anfragen:->:<[absender]>
        ...
        - case == remove:
            - if <[empfänger].flag[Profil.Freundesliste.Freunde].contains[<[absender]>]>:
                - flag <[absender]> Profil.Freundesliste.Freunde:<-:<[empfänger]>
                - flag <[empfänger]> Profil.Freundesliste.Freunde:<-:<[absender]>
        ...
        - case == accept:
            - if <[empfänger].flag[Profil.Freundesliste.Ausgang].contains[<[absender]>]>:
                - flag <[absender]> Profil.Freundesliste.Anfragen:<-:<[empfänger]>
                - flag <[empfänger]> Profil.Freundesliste.Ausgang:<-:<[absender]>
                - flag <[absender]> Profil.Freundesliste.Freunde:->:<[empfänger]>
                - flag <[empfänger]> Profil.Freundesliste.Freunde:->:<[absender]>
...
Freundes_Gui:
    ...
    - define online <list>
    - define offline <list>
    - foreach <player.flag[Profil.Freundesliste.Freunde]> as:f:
        - if <[f].is_online>:
            - define online:->:<[f]>
        - else:
            - define offline:->:<[f]>
    - foreach <[online].include[<[offline]>]> as:l:
        - if <[l].is_online>:
            - define name <[l].name>
            - define level <[l].flag[Profil.Level]>
            - define online <list>
